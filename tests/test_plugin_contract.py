...
def clean_registries():
    ...
    yield
    OPERATORS.clear()
    OPERATORS.update(ops_before)
    PROBES.clear()
    PROBES.update(probes_before)
...
def test_custom_operator_contract(clean_registries):
    def shout(target):
        ...
        return [
            Mutant(
                operator="shout",
                description="UPPERCASED the prompt",
    ...
    assert any(
        m.operator == "shout" and m.system.prompt == "BE QUIET PLEASE" for m in mutants
    )
...
def test_custom_eval_contract():
    ...
    ev = scorer_to_eval(
        lambda o, c: 1.0 if "$" in o else 0.0, threshold=0.5, name="price"
    )
    out = ev("costs $5", {})
...
def test_custom_probe_contract(clean_registries):
    ...
    cfg = MutEvalConfig(
        prompt="answer.",
        cases=[{"q": "x"}, {"q": "y", "unanswerable": True}],
        run=lambda p, c: "x",
